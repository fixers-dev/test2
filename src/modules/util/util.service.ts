...
export class UtilService {
  ...
  async sendKakao(phone: string, params: any, plus_id: string, template_code: string) {
    ...
    const body = {
      plusFriendId: process.env.KAKAO_PLUS_ID,
      templateCode: process.env.KAKAO_CODE,
    ...
    const options = {
      headers: {
        ...
        'X-Secret-Key': process.env.KAKAO_SECRET,
      },
      json: true,
    };

    const resultmsg = await this.httpPostRequest(
      `https://api-alimtalk.cloud.toast.com/alimtalk/v1.4/appkeys/${process.env.KAKAO_APP_KEY}/messages`,
      body,
      options,
    );
